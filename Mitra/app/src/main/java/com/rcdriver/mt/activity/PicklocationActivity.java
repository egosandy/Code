...
// IMPORT BARU untuk FusedLocationProviderClient
...
// import com.google.android.gms.location.LocationServices; // (ini adalah versi lama)
...
// PERBAIKAN: Hapus implementasi GoogleApiClient
public class PicklocationActivity extends AppCompatActivity
    ...
    // PERBAIKAN: Ganti GoogleApiClient dengan FusedLocationProviderClient
    private FusedLocationProviderClient mFusedLocationClient;

    private int formToFill;

    // PERBAIKAN: Hapus onStart dan onStop
    /*
    @Override
    ...
    */
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        // PERBAIKAN: Inisialisasi FusedLocationProviderClient
        mFusedLocationClient = LocationServices.getFusedLocationProviderClient(this);

        // PERBAIKAN: Sederhanakan dan amankan inisialisasi Places
        if (!Places.isInitialized()) {
            String apiKey = MainActivity.apikey;
            if (apiKey == null || apiKey.isEmpty()) {
                apiKey = getString(R.string.google_maps_key);
            }
            // Cukup panggil initialize saja.
            Places.initialize(getApplicationContext(), apiKey);
        }

        // PERBAIKAN: HAPUS PANGGILAN 'Places.createClient(this);' yang menyebabkan kebocoran
        // Places.createClient(this); // <-- HAPUS INI

        // PERBAIKAN: Hapus setupGoogleApiClient();
    ...
    */
    ...
    // PERBAIKAN: Ubah updateLastLocation menjadi Asynchronous
    @SuppressLint("MissingPermission") // Izin sudah dicek di awal metode
    private void updateLastLocation() {
        ...
        // Aktifkan tombol MyLocation di peta
        gMap.setMyLocationEnabled(true);

        // Panggil getLastLocation secara asynchronous (tidak memblokir UI Thread)
        mFusedLocationClient.getLastLocation().addOnSuccessListener(this, new OnSuccessListener<Location>() {
            @Override
            public void onSuccess(Location location) {
                // Panggilan balik ini terjadi ketika lokasi berhasil didapat
                if (location != null) {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        // Panggil metode baru yang sudah async
        updateLastLocation();
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == REQUEST_PERMISSION_LOCATION) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                // Jika izin diberikan, panggil lagi metode async
                updateLastLocation();
            }
        }
    }
                    ...
                    PicklocationActivity.this.runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            if (addresses != null && !addresses.isEmpty()) {
                                ...
                                textView.setText("Alamat tidak tersedia");
                            }
                        }
                    });
                } catch (IOException e) {
                    e.printStackTrace();
                    // Pastikan memberi feedback di UI Thread jika ada error
                    PicklocationActivity.this.runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            textView.setText("Gagal mendapatkan alamat");
                        }
                    });
                }
            }
    ...
    // PERBAIKAN: Hapus semua metode callback GoogleApiClient
    /*
    @Override
