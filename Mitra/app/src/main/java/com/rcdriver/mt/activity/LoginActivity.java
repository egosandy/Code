...
import com.rcdriver.mt.models.SettingsModel;
import com.rcdriver.mt.models.User;
import com.rcdriver.mt.utils.NetworkUtils;
...
public class LoginActivity extends AppCompatActivity {
    ...
    private void onSignInClick() {
        ...
        // 1. Dapatkan token FCM terlebih dahulu (ini proses asynchronous)
        FirebaseMessaging.getInstance().getToken()
                .addOnCompleteListener(new OnCompleteListener<String>() {
                    @Override
                    public void onComplete(@NonNull Task<String> task) {
                        if (!task.isSuccessful()) {
                            // Jika gagal mendapatkan token, hentikan proses dan beri notifikasi
                            progresshide();
                            Log.w("FCM", "Fetching FCM registration token failed", task.getException());
                            notif("Gagal mendapatkan token, silakan coba lagi.");
                            return;
                        }
                        ...
                        // 2. SETELAH TOKEN DIDAPATKAN, buat request login dan jalankan service
                        ...
                        request.setRegId(fcmToken); // Gunakan token yang baru didapat
                        ...
                        service.login(request).enqueue(new Callback<LoginResponseJson>() {
                            ...
                            public void onResponse(Call<LoginResponseJson> call, Response<LoginResponseJson> response) {
                                ...
                                if (response.isSuccessful()) {
                                    ...
                                    // Tambahkan penanganan jika response tidak successful (misal: error 404, 500)
                                    notif("Login gagal, server bermasalah.");
                                }
                            }
                            ...
                            public void onFailure(Call<LoginResponseJson> call, Throwable t) {
                                progresshide();
                                t.printStackTrace();
                                notif("Gagal terhubung ke server.");
                            }
                        });
                    }
                });
