...
class Driver extends CI_Controller
{
    ...
    public function detail($id)
    {
        ...
        $data['kendaraan'] = $this->driver->data_kendaraan_driver($id);
    ...
    public function detailk($id)
    {
        ...
        $data['driverjob'] = $this->driver->driverjob();
        $data['driver'] = $this->driver->data_kendaraan_driver_by_id($id);
        // echo json_encode($data);die();
        $this->load->view('includes/header', $data);
        $this->load->view('drivers/detail_kendaraan', $data);
        $this->load->view('includes/footer');
    }
    ...
    public function ubahdatak()
    {
        ...
        if ($this->form_validation->run() == TRUE) {
            $id = html_escape($this->input->post('id_driver', TRUE));
            
            $data             = [

                'id_k'                      => html_escape($this->input->post('id', TRUE)),
                'jenis'                     => html_escape($this->input->post('jenis', TRUE)),
                'merek'                     => html_escape($this->input->post('merek', TRUE)),
                'tipe'                      => html_escape($this->input->post('tipe', TRUE)),
                'nomor_kendaraan'           => html_escape($this->input->post('nomor_kendaraan', TRUE)),
            ...
            $this->driver->ubahkendaraan($data);
            $this->session->set_flashdata('ubah', 'Driver Vechile Has Been Changed');
            redirect('driver/detail/' . $id);
        } else {
