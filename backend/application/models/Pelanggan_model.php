...
class Pelanggan_model extends CI_model
{
 ...
 public function get_data_gmail_pelanggan($email)
    ...
    public function user_cancel_request($cond)
    {
        ...
        if ($id->row('status') == 1 || $id->row('status') == 2 || $id->row('status') == 9) {
            $data = array(
                'status' => '5'
            );
            if ($id2->row('home') == 4) {
...
public function merchantnearby($long, $lat)
    ...
    public function merchantbyid($idmerchant, $long, $lat)
    {
        ...
        $this->db->select('mitra.partner as partner');
        $this->db->select('category_merchant.nama_kategori');
    ...
    {
        $this->db->select('transaksi_item.jumlah_item,
        item.nama_item,item.id_item, 
        transaksi_item.catatan_item,
        transaksi_item.id_item, 
        transaksi_item.tipe,
        transaksi_item.nama_pesanan,
        transaksi_item.id_trans_item,
        transaksi_item.total_harga');
    ...
    public function update_transaksi_pending($id)
    {
        $data = [
            'status' => '9'
        ];
