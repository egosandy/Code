...
class Driver_model extends CI_model
{
    ...
    function insert_data_berkas($data_driver, $data_kendaraan, $data_berkas)
    {
        $inskendaraan = $this->db->insert('kendaraan', $data_kendaraan);
        $inserid = $this->db->insert_id();
        
        $dataupdate = array(
            'no_ktp' => $data_driver['no_ktp'],
            'foto' => $data_driver['foto'],
            'job' => $data_driver['job'],
            'kendaraan' => $inserid,
            'status' => $data_driver['status']
        );
        $this->db->where('id', $data_driver['id']);
        $signup = $this->db->update('driver', $dataupdate);

        $dataconfig = array(
            'id_driver' => $data_driver['id'],
            'latitude' => '0',
            'longitude' => '0',
            'status' => '5'
        );
    ...
    public function check_ktp($ktp)
    {
     ...
     public function check_stnk($stnk)
    {
        $cek = $this->db->query("SELECT id FROM driver where no_stnk='$stnk'");
        if ($cek->num_rows() == 1) {
            return true;
        } else {
            return false;
    ...
    public function get_data_driver_new($condition)
    {
        $this->db->where($condition);
    ...
    public function accept_request($cond)
    {
        ...
        $this->db->where('id_transaksi', $cond['id_transaksi']);
        $this->db->where("(status = '1' OR status = '9')", NULL, FALSE);
        $this->db->from('history_transaksi');
...
// 			$hasil = $kred + $hasil;
    ...
    function all_transaksi_pending()
    {
        $this->db->select('*');
        $this->db->from('transaksi');
        $this->db->join('transaksi_detail_merchant', 'transaksi.id = transaksi_detail_merchant.id_transaksi', 'left');
        ...
        $this->db->join('status_transaksi', 'history_transaksi.status = status_transaksi.id', 'left');
        $this->db->join('fitur', 'transaksi.order_fitur = fitur.id_fitur', 'left');
        $this->db->where('history_transaksi.status = 9');
        $this->db->order_by('transaksi.id', 'DESC');
        $trans = $this->db->get();
        return $trans;
    }
    ...
    public function ubahkendaraan($data)
    {
        $this->db->set('jenis', $data['jenis']);
        $this->db->set('merek', $data['merek']);
        $this->db->set('tipe', $data['tipe']);
    ...
    public function ubahdatacard($data, $data2)
    {
        ...
        $this->db->set('no_stnk', $data2['no_stnk']);
        $this->db->where('id', $data2['id']);
        $this->db->update('driver');
    }
...
function get_driver_location($idDriver)
  ...
  function my_token($data_l)
    ...
    {
        $this->db->where('id_trans_item', $data['id_trans_item']);
        return $this->db->update('transaksi_item', $data);
    }
    
    function get_item_transaksi($id)
    {
        $this->db->where('id_trans_item', $id);
        return $this->db->get('transaksi_item');
    }
    
    function data_kendaraan_driver($id)
    {
        $this->db->where('id_driver', $id);
        return $this->db->get('kendaraan')->result_array();
    }
    
    function data_kendaraan_driver_by_id($id)
    {
        $this->db->where('id_k', $id);
